...
- 每桌可以預先指定一份人員暱稱名單；設定後，賓客點餐時該桌就只能用名單裡的暱稱登入，避免打錯字或用到別人的位子（沒有設定名單的桌次則維持自由輸入暱稱）
...
**給參加聚餐的人（點餐）**
- 一進「我要點餐」就能看到目前所有桌次，以及每桌已指定的人員名單（如果有設定的話），點暱稱即可直接開始點餐；沒有指定名單的桌次可以自由輸入暱稱
- 如果同一個暱稱之前點過餐，會自動接續到上次的點餐紀錄，不用重新選桌
- 選擇自己坐哪一桌，點選菜色即可加入，同一道菜點兩次會自動累加數量；如果那桌人數已經到達上限，會提示滿了、無法再加入新的人
- 點餐頁面可以展開「查看各桌目前狀況」，看每一桌現在有哪些人、人數是否已滿，自己所在的那桌會特別標示出來
